...
class Board(Ui_MainWindow):  
    ...
    def squareClicked(self, isBot=False):
        ...
        # There was a weird first move bug which is really weird, so i implemented this code to make it fixed (not related to chess game set-up)
        ...
        # PGN Writer (edit PGN in case of check / checkmate).
        for plyr in Player.player_list:
            if plyr.isMyTurn: 
                for pc in Pieces.pieces:
        ...
        # Giving red color to squares which is involved for previous movement record. 
        if next_sequence:
            for sqr in Ui_MainWindow.boardSquare_list:
                if [sqr.coordinate[2], sqr.coordinate[1]] == prev_movement:
        ...
        # Bot and human-control are executed through this function. These lines are used to filter bot movement
...
class Player(Board): 
    ...
    def __init__(self, color, isBot = False): 
        ...
        # Player score GUI implementation.
    ...
    # Create and assign time to a signal.
    ...
    # Function executed under time-flow.
    def timeout(self): 
...
# Pieces class (function-wise, not object preference-wise).
class Pieces(Board):
    ...
    # Detect if any player runs out of time.
    def timeoutWin(): 
        winner_widget = QFrame(main_centralWidget)
        winner_widget.setFrameShape(QFrame.StyledPanel) 
        winner_widget.setLineWidth(100) 
